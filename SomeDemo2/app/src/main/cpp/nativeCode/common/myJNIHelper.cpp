...
MyJNIHelper::MyJNIHelper(JNIEnv *env, jobject obj, jobject assetManager, jstring pathToInternalDir,
    ...
    const char *cPathToInternalDir, *cSdCardPath;
    cPathToInternalDir = env->GetStringUTFChars(pathToInternalDir, NULL);
    cSdCardPath = env->GetStringUTFChars(jSdCardPath, NULL);
    apkInternalPath = std::string(cPathToInternalDir);
    sdcardPath = std::string(cSdCardPath);
...
std::string MyJNIHelper::getSDCardIndex(std::string basic_string) {
    return sdcardPath + basic_string;
}
